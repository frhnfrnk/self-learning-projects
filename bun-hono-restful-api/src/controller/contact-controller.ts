...
contactController.post("/api/contact", async (c) => {
...
contactController.get("/api/contact/:id", async (c) => {
  const user = c.get("user") as User;
...
contactController.put("/api/contact/:id", async (c) => {
  const user = c.get("user") as User;
  const contactId = Number(c.req.param("id"));
...
contactController.delete("/api/contact/:id", async (c) => {
  const user = c.get("user") as User;
  const contactId = Number(c.req.param("id"));
...
contactController.get("/api/contact", async (c) => {
  const user = c.get("user") as User;
